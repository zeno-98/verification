...
import ta.*;
import ta.dfa.DFA;
import ta.dfa.DfaLocation;
import ta.dfa.DfaTransition;
    ...
    public static DOTA getCartesian(DOTA ota, DFA dfa) {
        ...
        for (TaLocation l1 : ota.getLocations()) {
            for (DfaLocation l2 : dfa.getLocations()) {
                ...
                TaLocation taLocation = new TaLocation(id, name, init, accept);
        ...
        sigma.forEach(e -> {
            ...
            //第二种情况，只有ota存在的动作
            if (ota.containsSymbol(e) && !dfa.containsSymbol(e)) {
                asyncTransitions(clock, ota.getTransitions(null, e, null), dfa.getLocations(), pairLocationMap, newTransitions);
            }
            //第三种情况，只有dfa2存在的动作
            if (!ota.containsSymbol(e) && dfa.containsSymbol(e)) {
                asyncDfaTransitions(clock, dfa.getTransitions().stream().filter(t -> t.getSymbol().equals(e)).collect(Collectors.toList()),
                        ota.getLocations(), pairLocationMap, newTransitions);
            }
        });

        //构造笛卡尔积自动机DFA
        String name = ota.getName() + "_" + dfa.getName();
        return new DOTA(name, sigma, newLocations, newTransitions, clock);
    }
